...
public class GameManager : MonoSingleton<GameManager>
{
    ...
    //................navmesh agent.................................................................................................
    //private ThirdPersonCharacterModified TPCagent;
    ...
    void Start()
    {
        ...
        // assign the TPCagent
        //TPCagent = player.GetComponent<ThirdPersonCharacterModified>();
        ...
        GameObject bp = Instantiate(_buttonPromptCanvas);
        _buttonPrompt = bp.GetComponent<BottonPrompt>();
    ...
    //        len = calcpos.sqrMagnitude;    //calculate the square of the length
    //        if( len < (EventDistance * EventDistance))  //check if it is in range for event
    //        {
    //            InPosEvents[i].OnPos(); // Do the function
    //            StartCoroutine(AddToOutPosList(InPosEvents[i], 0.2f));  // add the position event to the check out list
    ...
    //        {
    //            StartCoroutine(AddToInPosList(OutPosEvents[i], 0.2f));  // add the position event to the check in list
    ...
    //private IEnumerator AddToInPosList(PosEvent posEvent, float time)
    //{
    //    while(time > 0)
    //    {
    //        time -= Time.deltaTime;
    //        yield return null;
    //    }
    //    InPosEvents.Add(posEvent);
    //}
    
    //private IEnumerator AddToOutPosList(PosEvent posEvent, float time)
    //{
    //    while(time > 0)
    //    {
    //        time -= Time.deltaTime;
    ...
    public void ShowInteractButtonPrompt(Vector3 pos, Vector3 inPos)
    {
        _buttonPrompt.gameObject.SetActive(true);
        _buttonPrompt.transform.position = pos + (Vector3.up * 1.5f);
        //_buttonPrompt.transform.LookAt(cam.transform);
        _buttonPrompt.transform.forward = -cam.transform.forward;
        Vector3 disp = inPos - pos;
        disp = Vector3.ProjectOnPlane(disp, cam.transform.forward);
        Vector3 projDown = Vector3.ProjectOnPlane(-_buttonPrompt.transform.up, cam.transform.forward);
        _buttonPrompt.Panel.localEulerAngles = new Vector3(0,0, -Vector3.SignedAngle(projDown.normalized, disp.normalized, cam.transform.forward));
    }
